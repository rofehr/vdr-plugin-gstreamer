...
public:
  ...
  const char *Version() { return "0.0.3"; }
  const char *Description() { return "GStreamer output plugin (AV)"; }
  bool ProcessArgs(int argc, char *argv[]) override {
    for (int i = 0; i < argc; ++i) {
      if (!strncmp(argv[i], "pipeline=", 9)) pipeline_desc_ = argv[i] + 9;
    }
    return true;
  }
  bool Start() override {
    if (pipeline_desc_.empty()) LoadPipelineConfig();
    device_ = new cGstDevice(pipeline_desc_);
    device_->SetEstimatedFps(25.0);
    device_->SetEstimatedAudioSampleRate(48000);
    return true;
  }
  void Stop() override { delete device_; device_ = nullptr; }
};
